...
import lothricKnights.WalkingForward.*;


public class WalkingForwardShield {
	...
	public static void animate(ArmorStand main, boolean walkingLeft) {
		try {
			...
			if (pelvis != null) {
				...
				if (Instant.now().isBefore(LothricKnights.animationTimer.get(pelvis).plusMillis(1000))) {
					pelvisPosition = new Vector(0,0.5,0);
				}
				//Up
				else if (Instant.now().isBefore(LothricKnights.animationTimer.get(pelvis).plusMillis(1300))) {
					pelvisPosition = pelvisPosition.add(new Vector(0,.04,0));	
				}
				else if (Instant.now().isBefore(LothricKnights.animationTimer.get(pelvis).plusMillis(1500))) {		
				}
				//Down
				else if (Instant.now().isBefore(LothricKnights.animationTimer.get(pelvis).plusMillis(1800))) {
					pelvisPosition = pelvisPosition.add(new Vector(0,-.04,0));	
				}
				else if (Instant.now().isBefore(LothricKnights.animationTimer.get(pelvis).plusMillis(2000))) {	
				}
				//Reset
				else if (Instant.now().isAfter(LothricKnights.animationTimer.get(pelvis).plusMillis(2000))) {
					LothricKnights.animationTimer.put(pelvis, Instant.now().plusMillis(-1000));
					pelvisPosition = new Vector(0,0.5,0);
				}
			...
			if (chest != null) {
				...
				//Animation
				//Combined angle of 25,35,0
				OutOfBounds.combined(chest, chest.getHeadPose(), 25, 25, 10, 35, 35, 10, 0, 0, 10);
			}
			...
			if (sword != null) {
				...
				//Animation
				//Combined angle of 0,0,0
				OutOfBounds.combined(sword, sword.getHeadPose(), 0, 0, 10, 0, 0, 10, 0, 0, 10);
			}
