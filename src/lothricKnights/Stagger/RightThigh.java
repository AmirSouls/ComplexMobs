...
public class RightThigh {
	public static void animate(ArmorStand part) {
		if (LothricKnights.animationTimer.containsKey(part)) {
			...
			OutOfBounds.combined(part, combAngle, -15, 0, 30, 0, 20, 30, 0, 0, 30);
			
			boolean debug = false;
			if (debug) {
			}
			else {
				//Start at 0,0,0 go to -15,20,0
				//Makes a flinch back like animation
				if (Instant.now().isBefore(LothricKnights.animationTimer.get(part).plusMillis(310))) {
					CombinedAnimation.animate(part, combAngle, -3, 4, 0);
				}
				else if (Instant.now().isBefore(LothricKnights.animationTimer.get(part).plusMillis(610))) {
					CombinedAnimation.animate(part, combAngle, .75, -1, 0);
				}
			}
		}
		else {
